...
export async function streamAgentTurn(args: {
  ...
  try {
    const ctx: AgentContext = {
      ...
      model: args.model,
      // Build 3 · AI Gateway request-tag context. Plant scope + environment are
      // env-driven (one plant per app instance); user_id is OBO-stamped inside
      // configureAgentsSdk from ctx.userEmail.
      plantId: process.env.VOLTA_PLANT_ID ?? 'PLANT-03',
      environment: process.env.VOLTA_ENVIRONMENT ?? 'demo',
      modelError,
