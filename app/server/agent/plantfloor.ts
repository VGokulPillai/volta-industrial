...
export type AgentContext = {
  ...
  model: string;
  /** Plant this app instance is scoped to (Build 3 AI Gateway request tag
   * `plant_id`). Defaults to the hero plant PLANT-03. */
  plantId?: string;
  /** Deployment environment for AI Gateway request tag `environment`
   * (`demo` / `prod`). */
  environment?: string;
...
export async function configureAgentsSdk(ctx: AgentContext): Promise<void> {
  ...
  const client = new OpenAI({
    ...
    fetch: async (input, init) => {
      ...
      // ── Build 3 · Unity AI Gateway — ATTRIBUTABLE ─────────────────────────
      // Tag every generative model request so its usage is attributable in
      // `system.ai_gateway.usage.request_tags`. Answers "who / which app /
      // which plant / which feature generated this AI spend?". Keys/values are
      // strings only; no secrets or unsafe PII (user_id is the OBO email, which
      // already governs the call). See submission/03_ai_gateway.
      headers.set(
        'Databricks-Ai-Gateway-Request-Tags',
        JSON.stringify({
          application: 'volta-maintenance',
          environment: ctx.environment ?? 'demo',
          plant_id: ctx.plantId ?? 'PLANT-03',
          feature: 'maintenance-assistant',
          user_id: ctx.userEmail || 'unknown',
        }),
      );
      let body = init?.body;
